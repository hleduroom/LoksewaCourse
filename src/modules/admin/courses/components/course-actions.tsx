...
import {
  deleteCourse,
  updateCourseToBePublished,
  updateCourseToBeUnPublished,
} from "../action";
...
const CourseActions = ({
  ...
  const onPublish = async () => {
    try {
      ...
      if (isPublished) {
        const result = await updateCourseToBeUnPublished(courseId);
        if (result.success) {
          toast.success(result.message);
          router.refresh();
        } else {
        ...
        const result = await updateCourseToBePublished(courseId);
        if (result.success) {
          toast.success(result.message);
          router.refresh();
        } else {
